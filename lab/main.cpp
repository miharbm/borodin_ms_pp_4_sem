...
int main(){
    ...
    double time_step = 0.012; // шаг по времени 0,012 сек
...
//    }
    ...
    int begin_pool = (int)(begin_pool_metres * 100.f);
    int end_pool   = (int)(end_pool_metres   * 100.f); // задание радиуса и длины трубы
    ...
    for ( int k = begin_pool; k < end_pool; k++ ){
        for (int i = (int)(tube1.diam() * 0.96); i > tube1.diam() * 0.9; i--){
            for (int j = 0; j < tube1.rad(); j++){
                if(!tube1.at(k, i, j - 1).type()) {
                    for (int l = j; l < 3; l++){
                        tube1.at(k, i ,j ).C(0.2);
                        tube1.at(k, i ,tube1.diam() - l).C(0.2);
                    }
                }
            }
        }
